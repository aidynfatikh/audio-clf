#!/usr/bin/env bash
# Run WavLM stage-1 (train) + stage-2 (finetune) on all 3 split configs, GPU 1.
...
# Force WavLM — .env may set BACKBONE=hubert for another session.
export BACKBONE=wavlm
TS=$(date +%Y%m%d-%H%M%S)

# label | split-dir | config
EXPERIMENTS=(
  "wavlm-01         |batch01_only_v1|configs/splits/batch01_only.yaml"
  "wavlm-01-02      |b1_b2_v1       |configs/splits/batch01_plus_batch02.yaml"
  "wavlm-01-02-kazemo|b1_b2_kazemo_v1|configs/splits/all_three.yaml"
)
